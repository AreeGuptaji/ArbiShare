...
export class DecentralizedPriceOracle {
    ...
    async getPrice(tokenSymbol, chainId) {
        ...
        try {
            ...
            console.error(`Error getting price for ${tokenSymbol} on chain ${chainId}:`, error);
            return null;
        }
    }
    // Get price from Pyth Network
    async getPythPrice(tokenSymbol, chainId) {
        try {
            const client = this.clients.get(chainId);
            const pythAddress = PYTH_ADDRESSES[chainId];
            if (!client || !pythAddress)
                return null;
            const tokenConfig = SUPPORTED_TOKENS[tokenSymbol];
            if (!tokenConfig?.pythPriceId)
                return null;
    ...
    async getUniswapPrice(tokenSymbol, chainId) {
        try {
            const client = this.clients.get(chainId);
            if (!client)
                return null;
            ...
            for (const baseToken of baseTokens) {
                const baseConfig = SUPPORTED_TOKENS[baseToken];
                const baseAddress = baseConfig?.addresses[chainId];
                if (!baseAddress)
                    continue;
                const poolAddress = await this.findUniswapPool(tokenAddress, baseAddress, chainId);
                if (!poolAddress)
                    continue;
                const price = await this.getPoolPrice(poolAddress, tokenAddress, baseAddress, chainId);
                if (price) {
    ...
    async findUniswapPool(token0, token1, chainId) {
        try {
    ...
    async getPoolPrice(poolAddress, token0, token1, chainId) {
        try {
            const client = this.clients.get(chainId);
            if (!client)
                return null;
    ...
    async getAggregatedPrice(tokenSymbol, chainId) {
        try {
            ...
            console.error(`Error getting aggregated price for ${tokenSymbol}:`, error);
            return null;
        }
    }
    ...
    getCacheStats() {
        ...
        const expired = Array.from(this.priceCache.values()).filter((item) => item.expiry <= now).length;
        return {
            total,
