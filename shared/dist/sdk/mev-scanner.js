...
import { mainnet, arbitrum, base, optimism, sepolia, arbitrumSepolia, baseSepolia, optimismSepolia, } from "viem/chains";
...
export const SUPPORTED_TOKENS = {
    ...
    USDC: {
        symbol: "USDC",
        decimals: 6,
        pythPriceId: "0xeaa020c61cc479712813461ce153894a96a6c00b21ed0cfc2798d1f9a9e9c94a", // USDC/USD
        addresses: {
...
export class DecentralizedMEVScanner {
    ...
    notifySubscribers() {
        this.subscribers.forEach((callback) => callback(this.getValidOpportunities()));
    }
            ...
            for (const [tokenSymbol, tokenConfig] of Object.entries(SUPPORTED_TOKENS)) {
                for (let i = 0; i < chainIds.length; i++) {
                    for (let j = i + 1; j < chainIds.length; j++) {
                        ...
                        if (!tokenConfig.addresses[sourceChain] ||
                            !tokenConfig.addresses[targetChain]) {
                            continue;
                        }
                        const opportunity = await this.checkCrossChainArbitrage(tokenSymbol, tokenConfig, sourceChain, targetChain);
                        if (opportunity) {
    ...
    async checkCrossChainArbitrage(tokenSymbol, tokenConfig, sourceChain, targetChain) {
        try {
            // Get prices from both chains using on-chain data
            const [sourcePrice, targetPrice] = await Promise.all([
                this.getTokenPrice(tokenConfig.addresses[sourceChain], sourceChain),
                this.getTokenPrice(tokenConfig.addresses[targetChain], targetChain),
            ]);
            ...
            return {
                ...
                tokenSymbol,
                tokenAddress: tokenConfig.addresses[sourceChain],
                sourceChain,
                targetChain,
                sourcePrice,
                targetPrice,
    ...
    async getTokenPrice(tokenAddress, chainId) {
        try {
            ...
            if (!tokenSymbol)
                return null;
            const basePrice = mockPrices[tokenSymbol];
            const variation = chainVariation[chainId] || 1.0;
            return basePrice * variation;
        }
        catch (error) {
            console.error(`Error getting price for ${tokenAddress} on chain ${chainId}:`, error);
            return null;
        }
    }
    ...
    async estimateGasCosts(sourceChain, targetChain) {
